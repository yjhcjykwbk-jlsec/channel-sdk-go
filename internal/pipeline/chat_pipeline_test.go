...
func TestChatPipelinePushReportsHandlerError(t *testing.T) {
	cfg := types.BatchConfig{
	...
	cp.SetErrorHandler(func(ctx context.Context, err error) {
		errCh <- err
	})

	cp.Push(context.Background(), &types.NormalizedMessage{
		MessageID: "om_error",
		Content:   "hello",
	}, func(ctx context.Context, dispatch *types.BatchedDispatch) error {
		return handlerErr
	})
	...
	case <-time.After(100 * time.Millisecond):
		t.Fatal("expected pipeline error handler to receive flush error")
	}
}
...
func TestChatPipelineManagerDisposeIsIdempotent(t *testing.T) {
	cpm := NewChatPipelineManager(types.BatchConfig{MaxMessages: 1, MaxChars: 4000})
	cpm.Push(context.Background(), "scope", &types.NormalizedMessage{
		MessageID: "om_dispose",
		Content:   "hello",
	}, func(ctx context.Context, dispatch *types.BatchedDispatch) error {
		return nil
	})
